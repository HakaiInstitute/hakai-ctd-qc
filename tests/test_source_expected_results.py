...
pytestmark = pytest.mark.skipif(
    source == "local", reason="Test suite is not from local"
)


@pytest.mark.parametrize("hakai_id", HAKAI_IDS_WITH_ISSUES["do_cap_fail_hakai_ids"])
def test_hakai_id_with_do_cap_issue(hakai_id, df_initial, source):
    data = df_initial.loc[df_initial['hakai_id'] == hakai_id]

    assert not data.empty, f"{source} No data found for hakai_id {hakai_id}"
    assert (
        data["dissolved_oxygen_ml_l_flag_level_1"] == 4
    ).all(), f"{source} dissolved_oxygen_ml_l_flag_level_1 for hakai_id {hakai_id} is not 4=FAIL"
    assert data["dissolved_oxygen_ml_l_flag"].str.startswith(
        "SVD"
    ).all(), f"{source} dissolved_oxygen_ml_l_flag for hakai_id {hakai_id} is not flagged as SVD"
    assert (
...
def test_hakai_ids_with_no_soak_warning(hakai_id, df_initial,source):
    data = df_initial.loc[df_initial['hakai_id'] == hakai_id]
    for variable in [
        ...
        ).all(), f"{source} {variable}_flag_level_1 for hakai_id {hakai_id} is not 3=WARNING"
        assert data[f"{variable}_flag"].str.startswith(
            "SVC"
        ).all(), f"{source} {variable}_flag for hakai_id {hakai_id} is not flagged as SVC"
        assert (
...
def test_hakai_ids_with_slow_oxygen_warning(hakai_id, df_initial,source):
    data = df_initial.loc[df_initial['hakai_id'] == hakai_id]
    ...
    assert data["dissolved_oxygen_ml_l_flag"].str.startswith(
        "SVC"
    ).all(), f"{source} dissolved_oxygen_ml_l_flag for hakai_id {hakai_id} is not flagged as SVC"
    assert (
...
def test_hakai_ids_with_short_static_deployment(hakai_id, df_initial,source):
    data = df_initial.loc[df_initial['hakai_id'] == hakai_id]
