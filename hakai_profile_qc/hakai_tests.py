...
# Read station depth list generated by Alex Hare
"""Alex Hare Feb 2023 - It turns out there are some gaps in the NONNA 10 m bathymetry, 
including in Burke and Dean channels where the depth issue recently arose. These gaps
 show up as '0.0' depth values. There are also some gaps in the NOAA 3 arcsecond values, 
 which seem to occur at locations within the tidal depth range (i.e., very close to shore). 
 So, I would recommend that you use the NONNA 10 m bathymetry where it's available, 
 and use the NOAA 3 arcsecond bathymetry everywhere else (i.e., where the NONNA 10 m 
 values are 0.0). Any station with missing depth information (which would be from 
 the NOAA bathy) is likely intertidally located, and I'm not sure how you handle 
 that but I assume you've got something figured out, or the depth-in-station test 
 doesn't apply there."""

hakai_stations = pd.read_csv(
    os.path.join(os.path.dirname(__file__), "Hakai_CTD_stations_metadata.csv")
).rename(columns={"Station": "station"})
hakai_stations["station_depth"] = (
    -hakai_stations["NONNA_10m_"]
    .replace({0: None})
    .fillna(hakai_stations["NOAA_3arcs"])
)
