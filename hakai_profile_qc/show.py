...
def test_results_bar_plot(
    ...
    df_test = (
        ...
        .stack()
        .reset_index()
        .rename({"level_1": "Test", 0: "Flag"}, axis=1)
    )
    df_test["Count"] = 1

    # Regroup data by test and flag and count how many values there is per flag
    df_result = (
        df_test.groupby(["Test", "Flag"])["Count"].count().dropna().reset_index()
    )
    ...
    if (
        df_result["Flag"].dtype is object
    ):  # Convert Hakai flags that already exist in the database to QARTOD equivalent
...
def interactive_profile_viewer(df, variable_list, test_list, var_to_plot=None):
    if var_to_plot is None:
        var_to_plot = tuple(set(variable_list) - {"depth", "pressure"})

    # Define widgets
    hakai_ids = df["hakai_id"].unique()
    hakai_id = hakai_ids[0]
    flag_type = widgets.Dropdown(
        options=test_list, value="None", description="Test to review", disabled=False
    )
    considered_flag = widgets.SelectMultiple(
        options=[1, 2, 3, 4, 9],
        value=[1, 2, 3, 4, 9],
        description="Flag To Consider",
        disabled=False,
    )
    consider_only_downcast = widgets.Checkbox(
        value=True, description="Consider Downcast Only", disabled=False, indent=False
    )
    vars = widgets.SelectMultiple(
        options=variable_list - {"depth", "pressure"},
        value=var_to_plot,
        description="Ocean Variable",
        disabled=False,
    )
    y_axis = widgets.Select(
        options=variable_list,
        value="depth",
        description="Y Axis",
        disabled=False,
    )
    select_downcast = widgets.Checkbox(
        value=True, description="Downcast", disabled=False
    )
    select_upcast = widgets.Checkbox(value=True, description="Upcast", disabled=False)
    par_log = widgets.Checkbox(value=True, description="PAR Log Scale", disabled=False)
    hakai_id_list_menu = widgets.Select(
        options=hakai_ids,
        value=hakai_id,
        # rows=10,
        description="Hakai ID",
        disabled=False,
    )
    # Widgets Layout
    flag_selection = VBox((flag_type, considered_flag, consider_only_downcast))
    plotting_selection = HBox((hakai_id_list_menu, vars, y_axis))
    checkboxes = VBox((select_downcast, select_upcast, par_log))
    upper_menu = HBox((flag_selection, plotting_selection, checkboxes))

    def which_profiles_to_look_at(
        flag_type=flag_type,
        considered_flag=considered_flag,
        consider_only_downcast=consider_only_downcast,
    ):
        ...
        if flag_type == "None":
            ...
            hakai_id_list = df_selected[df_selected[flag_type].isin(considered_flag)][
                "hakai_id"
            ].unique()
        hakai_id_list_menu.options = hakai_id_list

    def plot_profile(
        hakai_id=hakai_id_list_menu,
        ocean_variables=vars,
        y_axis=y_axis,
        downcast=select_downcast,
        upcast=select_upcast,
        par_log_scale=par_log,
    ):
        ...
        # Get hakai_id data
        df_temp = df[df["hakai_id"] == hakai_id].sort_values(
            ["direction_flag", "depth"]
        )

        # Create Subplots
        fig = make_subplots(
            rows=1,
            cols=len(ocean_variables),
            shared_yaxes=True,
        ...
        fig.update_layout(
            ...
            + hakai_id
            + " Site: "
            + df_temp["station"].unique()[0],
        )
        return fig.show()

    out1 = widgets.interactive_output(
        which_profiles_to_look_at,
        {
            "flag_type": flag_type,
            "considered_flag": considered_flag,
            "consider_only_downcast": consider_only_downcast,
    ...
    display(upper_menu, out1, out2)


def flag_result_map(df, flag_variable="position_qartod_flag", groupby_var="hakai_id"):
    # Start the map with center on the average lat/long
    center_map = (
        df.groupby(groupby_var)[["latitude", "longitude"]].mean().mean().to_list()
    )

    # Start by defining the map
    m = folium.Map(
        location=center_map, zoom_start=9, control_scale=True, tiles="Stamen Terrain"
    )
    # Create groups
    ...
    for index, row in df[df[flag_variable] == 3].groupby(by=groupby_var):
        f3.add_child(
            folium.Marker(
                row[["latitude", "longitude"]].mean().tolist(),
                popup="[SUSPECT] hakai_id: " + str(index),
    ...
    for index, row in df[df[flag_variable] == 9].groupby(by=groupby_var):
        f9.add_child(
            folium.Marker(
                row[["latitude", "longitude"]].mean().tolist(),
                popup="[UNKNOWN] hakai_id: " + str(index),
    ...
    f1.add_child(
        folium.plugins.FastMarkerCluster(
            df[df[flag_variable] == 1]
            .groupby(by=groupby_var)
...
def flag_result_plot(
    ...
    for hakai_id in hakai_id_to_plot:
        print(hakai_id)
        plt.figure()
        fig, axs = plt.subplots(1, len(variables_to_plot), sharex=False, sharey=True)
